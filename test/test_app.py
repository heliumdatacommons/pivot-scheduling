...
class ApplianceTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.app = Appliance(env, 'test', contrs)

  def test_empty_appliance(self):
    try:
      Appliance(self.env, 'empty', [])
    except:
      self.fail('Failed to create empty appliance')

  def test_appliance_with_one_container(self):
    try:
      Appliance(self.env, 'one-container', [self.app.containers[0]])
    except:
      self.fail('Failed to create an appliance with only one container')
  ...
  def test_appliance_with_cycle(self):
    contrs = sorted(self.app.containers, key=lambda c: int(c.id))[:3]
    contrs[0].add_dependencies('1', '2')
    with self.assertRaises(ValueError):
      Appliance(self.env, 'app-with-cycle', contrs)

  def test_appliance_with_no_dependencies(self):
    env = self.env
    app = Appliance(env, 'test',
...
class RandomDataflowGeneratorTest(unittest.TestCase):
  ...
  def setUp(self):
    from appliance.gen import RandomDAGGenerator
    self.dag = RandomDAGGenerator(100, 100, .1, .1).generate()

  # def test_dataflow_validity(self):
  #   from appliance.gen import RandomDataflowGenerator
  #   dag = self.dag
...
class RandomApplianceGeneratorTest(unittest.TestCase):

  def setUp(self):
    from appliance.gen import RandomApplianceGenerator
    env = simpy.Environment()
    self.gen = RandomApplianceGenerator(env, n_nodes_lo=2, n_nodes_hi=20,
  ...
  def test_appliance_generation(self):
    app = self.gen.generate()
    self.assertIsInstance(app, Appliance)
